...
def format_ugui_runtime_click_text(result: dict[str, Any]) -> str:
    lines: list[str] = []
    header_parts = [
        f"dryRun:{format_toon_atom(result.get('dryRun'))}",
        f"playMode:{format_toon_atom(result.get('playMode'))}",
        f"mutation:{format_toon_atom(result.get('allowStateMutation'))}",
    ]
    lines.append("click " + " ".join(header_parts))

    target = result.get("target")
    if isinstance(target, dict):
        target_parts = [f"path:{format_toon_atom(target.get('path'))}"]
        instance_id = target.get("instanceId")
        if not should_omit_toon_value(instance_id):
            target_parts.append(f"id:{format_toon_atom(instance_id)}")
        enabled = target.get("enabled")
        if not should_omit_toon_value(enabled):
            target_parts.append(f"enabled:{format_toon_atom(enabled)}")
        interactable = target.get("interactable")
        if not should_omit_toon_value(interactable):
            target_parts.append(f"interactable:{format_toon_atom(interactable)}")
        controls = target.get("controlComponents")
        if isinstance(controls, list) and controls:
            target_parts.append("controls:" + ",".join(format_toon_atom(item) for item in controls if not should_omit_toon_value(item)))
        lines.append("target " + " ".join(target_parts))

    coordinate = result.get("coordinateConvention")
    if isinstance(coordinate, dict):
        pos_parts = []
        screen_position = format_vector2_pair(coordinate.get("screenPosition"))
        if screen_position:
            pos_parts.append(f"px:{screen_position}")
        normalized = format_vector2_pair(coordinate.get("normalizedPosition"))
        if normalized:
            pos_parts.append(f"norm:{normalized}")
        if coordinate.get("normalizedInputSupplied") is True:
            pos_parts.append("input:normalized")
        if pos_parts:
            lines.append("pos " + " ".join(pos_parts))

    handler = result.get("intendedHandler")
    if isinstance(handler, dict):
        handler_parts = []
        sequence = handler.get("sequence")
        if not should_omit_toon_value(sequence):
            handler_parts.append(f"sequence:{format_toon_atom(sequence)}")
        path = handler.get("path")
        if not should_omit_toon_value(path):
            handler_parts.append(f"path:{format_toon_atom(path)}")
        component = handler.get("component")
        if not should_omit_toon_value(component):
            handler_parts.append(f"component:{format_toon_atom(component)}")
        if handler_parts:
            lines.append("handler " + " ".join(handler_parts))

    selected_after = result.get("selectedObjectAfter")
    if isinstance(selected_after, dict) and not should_omit_toon_value(selected_after.get("path")):
        lines.append(
            "selectedAfter "
            + f"path:{format_toon_atom(selected_after.get('path'))} id:{format_toon_atom(selected_after.get('instanceId'))}"
        )
    ...
    if isinstance(stack, list) and stack:
        ...
        if isinstance(top, dict):
            lines.append(
                "hit "
                + f"path:{format_toon_atom(top.get('path'))} "
                + f"index:{format_toon_atom(top.get('stackIndex'))}"
            )
